...
Tensor slogdet_backward(const Tensor& grad_logabsdet,
  ...
  //auto singular = singular_case_backward(grad_logabsdet, self);
  //std::cout << nonsingular.dim() << singular.dim() << signdet.abs().dim() << std::endl;
  //return (signdet.abs().unsqueeze(1).unsqueeze(2) * nonsingular) + ((1 - signdet.abs()).unsqueeze(1).unsqueeze(2) * singular);
  return nonsingular;
}
